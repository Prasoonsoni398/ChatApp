...
const Chat = () => {
  ...
  const {
    ...
    handleTypingEmit,
    handleTypingReceive,
  } = useTypingIndicator(selectedChat, loggedInUser);
  ...
  const handleReply = () => {
    ...
    const senderName = contextMenu.isMe
      ? "You"
  ...
  const confirmForward = async (targetChat) => {
    ...
    try {
      ...
      if (forwardMessage.text)
        formData.append("text", `↗ Forwarded: ${forwardMessage.text}`);
  ...
  const forwardSelectedMessages = async (targetChat) => {
    ...
    toast.success(
      `Forwarded ${selectedMessageIds.length} message(s) to ${targetChat.name}`,
    );
    cancelSelectMode();
    setShowForwardModal(false);
    setForwardMessage(null);
      ...
      prev.map((m) => {
        if (ids.includes(m._id)) {
          if (type === "me")
            return {
              ...m,
              deletedFor: [...(m.deletedFor || []), loggedInUser._id],
            };
  ...
  useEffect(() => {
    ...
    const handleReceive = (msg) => {
      ...
      const matchesGroup =
        selectedChat.isGroup && msg.groupId === selectedChat.id;
  ...
  return (
    <div className="flex h-screen bg-base-200 overflow-hidden text-base-content">
      {/* ══ SIDEBAR ══ */}
      <div
        className={`w-full md:w-88 lg:w-96 flex-shrink-0 flex flex-col bg-base-100 border-r border-base-300 ${selectedChat ? "hidden md:flex" : "flex"}`}
      >
        {/* Sidebar Header */}
            ...
            .map((chat) => (
              ...
              >
                <div className="avatar">
                  <div
                    className={`w-12 rounded-full relative ${selectedChat?.id === chat.id ? "ring ring-primary ring-offset-base-100 ring-offset-2" : ""}`}
                  >
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex justify-between items-baseline mb-0.5">
                    <h3
                      className={`font-semibold truncate ${selectedChat?.id === chat.id ? "text-primary" : ""}`}
                    >
                      {chat.name}
                    </h3>
                            ...
                            clearUndoTimeoutRef.current = setTimeout(
                              async () => {
                                ...
                                try {
                                  const token = localStorage.getItem("token");
                                  await fetch(
                                    `/api/messages/clear/${selectedChat.id}`,
                                    {
                                      method: "DELETE",
                                      headers: {
                                        Authorization: `Bearer ${token}`,
                                  ...
                                  console.error(
                                    "Error clearing chat from backend",
                                    e,
                                  );
                                }
                              },
                  ...
                  <input
                    ...
                    onKeyDown={(e) => {
                      ...
                      if (e.key === "Enter" || e.key === "ArrowDown")
                        nextIdx = (msgSearchIndex + 1) % matches.length;
                      else if (e.key === "ArrowUp")
                        nextIdx =
                          (msgSearchIndex - 1 + matches.length) %
                          matches.length;
                ...
                {msgSearchQuery.trim() && (
                  <span className="text-xs text-base-content/50 whitespace-nowrap">
                    {(() => {
                      ...
                      return c > 0 ? `${msgSearchIndex + 1}/${c}` : "0 results";
                    })()}
                  </span>
                )}
                <button
                  ...
                  .map((msg, idx, filteredMessages) => {
                    ...
                    (msg.reactions || []).forEach((r) => {
                      if (!reactionMap[r.emoji])
                        reactionMap[r.emoji] = { count: 0, isMine: false };
                      reactionMap[r.emoji].count++;
                      if (
                        r.userId === loggedInUser?._id ||
                        r.userId?.toString() === loggedInUser?._id
                      )
                        reactionMap[r.emoji].isMine = true;
                    });
                        ...
                        >
                          ...
                          >
                            ...
                            >
                              {isTombstone ? (
                                ...
                                <>
                                  ...
                                  {msg.replyToText && (
                                    <div
                                      className={`border-l-4 rounded-lg px-2 py-1 mb-2 text-xs cursor-pointer ${isMe ? "border-white/60 bg-white/10" : "border-primary/60 bg-primary/10"}`}
                                    >
                                      <p
                            ...
                            {showFullEmojiForMsg === msg._id && (
                              <div
                                className={`absolute z-[150] top-0 animate-fade-in ${isMe ? "right-full mr-2" : "left-full ml-2"}`}
                              >
                                <EmojiPicker
                                  height={350}
                                  width={280}
                  ...
                  {showMentionPopup && selectedChat?.isGroup && (
                    <div className="absolute bottom-[calc(100%+8px)] left-0 w-64 max-h-48 overflow-y-auto bg-base-100 border border-base-300 rounded-xl shadow-2xl z-[150] animate-fade-in py-1">
                      {(selectedChat.members || [])
                        .filter(
                          (m) => m && m._id !== loggedInUser?._id && m.name,
                        )
                        .filter((m) =>
                          m.name
                            .toLowerCase()
            ...
            <>
              ...
              <li>
                <a
                  ...
                  className={contextMenuAction}
                >
                  <BsForward size={15} className="text-base-content/50" />{" "}
                  Forward
                </a>
              </li>
              <li>
